...
// SnapshotCounters captures current hardware utilization for phase boundaries.
func SnapshotCounters() map[string]float64 {
	...
	if err == nil {
		...
		if len(parts) >= 2 {
			...
			fmt.Sscanf(strings.TrimSpace(parts[0]), "%f", &gpuUtil)
			fmt.Sscanf(strings.TrimSpace(parts[1]), "%f", &gpuMem)
			counters["gpu_util_pct"] = gpuUtil
			counters["gpu_mem_used_mb"] = gpuMem
		}
	}
